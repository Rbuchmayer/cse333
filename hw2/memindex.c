...
LinkedList MIProcessQuery(MemIndex index, char *query[], uint8_t qlen) {
  ...
  while (!HTIteratorPastEnd(it)) {
    struct SearchResult* r0 = (SearchResult*) malloc(sizeof(SearchResult));
    Verify333(r0 != NULL);
    HTIteratorGet(it, &kv);
    r0->docid = kv.key;
    r0->rank = NumElementsInLinkedList(kv.value);
    AppendLinkedList(retlist, r0);
    HTIteratorNext(it);
  }
  HTIteratorFree(it);
  ...
  for (i = 1; i < qlen; i++) {
    ...
    for (j = 0; j < ne; j++) {
      SearchResult *r;
      LLIteratorGetPayload(llit, (void **) &r);
      res = LookupHashTable(wds->docIDs, r->docid, &kv);
      if (res == 0) {
	LLIteratorDelete(llit, &free);
      } else {
	r->rank += NumElementsInLinkedList(kv.value);
	LLIteratorNext(llit);
      }
    }
    LLIteratorFree(llit);
