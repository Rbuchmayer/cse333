...
DocID_t DTRegisterDocumentName(DocTable table, char *docname) {
  ...
  // STEP 2.
  HTKey_t hash = FNVHash64((unsigned char *) doccopy, strlen(doccopy));
  int r = LookupHashTable(table->docname_to_docid, hash, &oldkv);
  if (r == 1) {
    free(doccopy);
    free(docid);
...
DocID_t DTLookupDocumentName(DocTable table, char *docname) {
  ...
  // docID if it is.
  res = LookupHashTable(table->docname_to_docid, FNVHash64((unsigned char*) docname, strlen(docname)), &kv);
  if (res == 0) {
    return 0;
  }
